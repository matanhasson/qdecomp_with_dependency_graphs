...
local decode_strategy = std.extVar('decode_strategy'); // "operators_mask" | "probs_multiplication"
local tags_namespace = "labels";
local operators_namespace = "operators_labels";

//hyperparameters
local seed = std.parseInt(std.extVar('seed')); //24;
...
local operator_representation_dim = std.parseInt(std.extVar('operator_representation_dim')); // 300;
local tag_representation_dim = std.parseInt(std.extVar('tag_representation_dim')); //300;
local operator_ff_num_layers = std.parseInt(std.extVar('operator_ff_num_layers')); // 1;
local tag_ff_num_layers = std.parseInt(std.extVar('tag_ff_num_layers')); // 1;
